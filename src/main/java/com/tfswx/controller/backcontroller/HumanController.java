...
public class HumanController extends BaseController {
    ...
    public String sortFront(@PathVariable String lastId,@PathVariable String id,Model model){
        humanService.sortFront(CommonUtils.restFulConverter(lastId),CommonUtils.restFulConverter(id));
        model.addAttribute("id",3);
        return "back/index";
    }
    ...
    public String sortDown(@PathVariable String nextid,@PathVariable String id,Model model){
        humanService.sortDown(CommonUtils.restFulConverter(nextid),CommonUtils.restFulConverter(id));
        model.addAttribute("id",3);
        return "back/index";
    }
}
