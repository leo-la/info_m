...
public class LawController extends BaseController {
    ...
    public String sortFront(@PathVariable String lastId,@PathVariable String id,Model model){
        lawService.sortFront(CommonUtils.restFulConverter(lastId),CommonUtils.restFulConverter(id));
        model.addAttribute("id",8);
        return "back/index";
    }
    ...
    public String sortDown(@PathVariable String nextid,@PathVariable String id,Model model){
        lawService.sortDown(CommonUtils.restFulConverter(nextid),CommonUtils.restFulConverter(id));
        model.addAttribute("id",8);
        return "back/index";
    }
}
