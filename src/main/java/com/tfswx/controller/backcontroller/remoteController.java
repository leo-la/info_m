...
public class remoteController extends BaseController {
    ...
    public String sortFront(@PathVariable String lastId,@PathVariable String id,Model model){
        remoteService.sortFront(CommonUtils.restFulConverter(lastId),CommonUtils.restFulConverter(id));
        model.addAttribute("id",11);
        return "back/index";
    }
    ...
    public String sortDown(@PathVariable String nextid,@PathVariable String id,Model model){
        remoteService.sortDown(CommonUtils.restFulConverter(nextid),CommonUtils.restFulConverter(id));
        model.addAttribute("id",11);
        return "back/index";
    }
}
