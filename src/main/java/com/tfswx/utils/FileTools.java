...
//            FileTools.copyFile(filePath, newSrc, true);
//        }
    ...
    public static boolean copyDirectory(String srcDirName, String destDirName, boolean overlay) {
        ...
        for (int i = 0; i < files.length; i++) {
            // 复制文件
            if (files[i].isFile()) {
                flag = FileTools.copyFile(files[i].getAbsolutePath(), destDirName + files[i].getName(),
                        overlay);
                if (!flag) {
                    break;
                }
    ...
    public static final StringBuffer getHtmlCss(String src) throws IOException {
        ...
        for (Element style : links) {
            ...
            StringBuffer link = FileTools.readFile(realPath);
